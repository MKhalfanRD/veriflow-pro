...
import { formatTanggal, STATUS_LABEL, CURRENT_BALAI } from "@/lib/mock-data";
...
function Page() {
  ...
  const allEvents: Event[] = usulan.flatMap((u) => {
    const items: Event[] = [
      {
        ...
        <div className="px-6 py-4 border-b border-border">
          ...
          <p className="text-[11px] text-muted-foreground">
            {role === "balai"
              ...
              : role === "verif1"
              ? "Pengajuan & verifikasi tahap teknis"
              : role === "verif2"
              ? "Seluruh aktivitas — dikelompokkan per role"
              : "Aktivitas pengajuan publik"}
          </p>
        </div>
